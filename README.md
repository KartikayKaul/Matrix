...
Be careful when dealing with `std::complex` matrices. Although I have tested the library with complex matrices and it works fine in some cases but when it comes to performing operations where type conversion might occur due to there being explicit conversion or implicit, some errors may arise. Still I am actively taking into consideration adding safety measures to handle such cases and accomodate operations for complex numerical operations with the matrices.
